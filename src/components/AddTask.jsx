...
function AddTask() {
  ...
  const [formData, setFormData] = React.useState({
    ...
    task: '',
    completed: false,
    dueDate: '' // Changed from 'time' to 'dueDate'
  });

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:4000/api/task/addTask', formData);
      ...
      <div className='h-auto overflow-hidden p-8 text-center'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col space-y-4 items-center justify-center mt-8'>
          ...
          <input
            ...
            value={formData.dueDate} // Changed from 'time' to 'dueDate'
            required
